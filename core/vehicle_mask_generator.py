...
class VehicleMaskGenerator(StaticRegionDetector):
    """Alias for StaticRegionDetector for UI compatibility."""
    ...
    def generate_mask(
        ...
        """
        Generate vehicle mask from image paths.
        ...
        Returns:
            (mask, statistics) where mask is binary (0 or 255)
        """
        # Sample images
        num_samples = min(num_samples, len(image_paths))
        sampled_files = random.sample(image_paths, num_samples)

        print(f"Loading {num_samples} images from {len(image_paths)} total...")
        ...
        # Detect static regions
        vehicle_mask, stats = self.detect_static_regions(images)

        # Convert to 0/255 format
        vehicle_mask = vehicle_mask * 255
